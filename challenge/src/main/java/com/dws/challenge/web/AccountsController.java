...
public class AccountsController {
	...
	public ResponseEntity<Object> amountTransfer(@RequestBody @Valid FundTransfer fundTransfer) {
		log.info("fundTransfer Details {}", fundTransfer);

		try {
			this.accountsService.transferFund(fundTransfer);
		} catch (Exception tranEx) {
			return new ResponseEntity<>(tranEx.getMessage(), HttpStatus.BAD_REQUEST);
		}
